...
# This will render the Home page when requested
def home(request):
...
# will render the create new account page when requested
def create_account(request):
    form = AccountForm(data=request.POST or None)  # retrieve the account form
    # checks if request method is POST
    if request.method == 'POST':
        if form.is_valid():  # checks to see if form is valid if so saves the form
...
# This will render the Balance page when requested
...
# will render the transaction page when requested
